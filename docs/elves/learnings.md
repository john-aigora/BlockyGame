...
- [2026-08-08] Wall-clock→game-clock conversion ended the load-flake era: 17 sites converted (B9) took full-suite flake from ~1-in-3 to zero across the whole (~150+-test) suite. `debug.advance(seconds)` (rAF-paused real-update pump) makes time-dependent specs cheap. CLAUDE.md owns the exact test count.
...
- [2026-07-25, amended 2026-08-15] No PR bots review this repo (CI runs the suite; humans + run reviewers judge). The review loop is a fresh read-only subagent per batch given the diff, the plan file, the contract, and today's date. It must check plan Done-criteria completeness, not just code quality. (Runs DO push to the fork and open PRs now — see Known Traps.)
...
- [2026-07-25, amended 2026-08-15] The end-screen title is written per show (plan 029): DEATH shows always restore exactly "GAME OVER" (smoke/gameover/coop/hiscores specs assert the exact text); an ascension end shows "ASCENDED!". Growth ends at ASCENSION_SCALE — new features must honor an ascending player's stand-down (grep `player.ascension`).
...
- [2026-07-25, superseded 2026-08-15] Direct push to origin is DENIED (account RBrownHOPE is read-only on john-aigora/BlockyGame) — but the old "never push" rule is DEAD: the live workflow is push to the FORK (remote `fork`, RBrownHOPE/BlockyGame) and land via PR (precedent: PRs #4, #5, #19). Runs push their own feature branch to the fork; merge stays user-owned.
...
<!-- Plan 032 (DOC-12): five LIVE lessons were misfiled under this heading
     with no retirement markers — an agent skimming section titles concluded
